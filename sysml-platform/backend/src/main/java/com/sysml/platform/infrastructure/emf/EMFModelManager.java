...
public class EMFModelManager {
  ...
  public String saveObject(EObject object) {
    try {
      String path = "/emf/" + object.eClass().getName() + "/" + System.currentTimeMillis();
      String cdoUri = cdoModelService.saveModel(object, path);
      log.info("Saved object to CDO: {}", cdoUri);
      return cdoUri;
    } catch (Exception e) {
  ...
  /** 查询对象 */
  public EObject findObject(String path) {
    try {
      return cdoModelService.loadModel(path);
    } catch (Exception e) {
      log.error("Failed to load object from path: {}", path, e);
      return null;
    }
  }
